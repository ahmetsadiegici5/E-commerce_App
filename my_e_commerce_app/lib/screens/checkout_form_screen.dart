...
class _CheckoutFormScreenState extends State<CheckoutFormScreen> {
  ...
  void _openPaymentPage() async {
    if (kIsWeb) {
      ...
      // Mobil platformlarda URL launcher ile aç
      final Uri uri = Uri.parse(widget.checkoutFormUrl);
      if (await canLaunchUrl(uri)) {
        await launchUrl(uri, mode: LaunchMode.externalApplication);
        if (mounted) {
          Navigator.of(context).pop(false); // Sonucu bilemediğimiz için false döndür
        }
      } else {
        if (mounted) {
  ...
  void _monitorIyzicoPaymentWindow(html.WindowBase? paymentWindow) {
